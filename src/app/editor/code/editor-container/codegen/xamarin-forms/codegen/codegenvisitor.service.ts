...
export abstract class CodeGenVisitor {
    ...
    protected colorRatioToHex(red: number, green: number, blue: number) {
      return '#' + Math.round(red * 255).toString(16).padStart(2, '0')
    ...
    private checkForBorder(parent: SketchMSLayer, ast: SketchMSLayer): string | boolean {
      if ( ast.frame.x === 0
        ...
        && !!ast.style.borders) {
          ...
          return this.colorRatioToHex(ast.style.borders[0].color.red,
                                      ast.style.borders[0].color.green,
                                      ast.style.borders[0].color.blue);
      }
      return false;
    }
